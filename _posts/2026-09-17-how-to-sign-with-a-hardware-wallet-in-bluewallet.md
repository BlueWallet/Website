...
This post does one job: send from a PSBT-compatible hardware wallet using BlueWallet as watch-only. If you wanted a [Vault](/how-to-create-a-multisig-vault-in-bluewallet/) instead, that how-to already shipped. If you wanted an [old phone as the cold side](/old-phone-bitcoin-cold-wallet/), that lives in the [offline docs](/docs/offline-cold-wallet/).

## What you need

- BlueWallet on the phone you carry. Official builds on the [download page](/download/).
...
Do not import the recovery phrase onto the phone "just to make it easier." That turns the hardware wallet into expensive jewelry. Watch-only is the path: [How to set up a watch-only wallet in BlueWallet](/how-to-set-up-a-watch-only-wallet-in-bluewallet/).

## Put the hardware wallet on BlueWallet as watch-only

Same three taps as [How to set up a watch-only wallet in BlueWallet](/how-to-set-up-a-watch-only-wallet-in-bluewallet/).
...
On many devices the export is labeled something like Electrum / Native SegWit / XPUB. zpub is the usual pair for Native SegWit. You can also pull it later from Wallet details, then Show Wallet XPUB, on any BlueWallet that already holds the keys offline.
...
If broadcast fails, the signature is usually fine and the network is busy or the fee is too low. The coins are not gone. You still have a signed transaction you can try again or [broadcast](/docs/broadcast-transaction/) later.
...
If you need more than one key to spend, that is a [Vault](/how-to-create-a-multisig-vault-in-bluewallet/), and the hardware wallet can be one of the cosigners.

If you have no hardware wallet and an old phone to spare, [Use BlueWallet offline as a cold wallet](/docs/offline-cold-wallet/) and [How to Turn an Old Phone Into a Bitcoin Cold Wallet](/old-phone-bitcoin-cold-wallet/) are the same air-gap idea without buying a new gadget.

---

*Related reading: [Sign a transaction offline](/docs/sign-offline/) · [How to set up a watch-only wallet in BlueWallet](/how-to-set-up-a-watch-only-wallet-in-bluewallet/) · [Use BlueWallet offline as a cold wallet](/docs/offline-cold-wallet/) · [How to Turn an Old Phone Into a Bitcoin Cold Wallet](/old-phone-bitcoin-cold-wallet/) · [How to create a multisig vault in BlueWallet](/how-to-create-a-multisig-vault-in-bluewallet/)*
